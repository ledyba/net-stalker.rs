...
#[derive(Default)]
pub struct JamstecBase;

impl Site for JamstecBase {
  fn fetch(&self) -> Pin<Box<dyn Future<Output=anyhow::Result<String>> + Send>> {
    Box::pin(async {
      let content = reqwest::get("https://www.jamstec.go.jp/j/pr/")
        .await?
...
fn build_rss(doc: &scraper::Html) -> anyhow::Result<Channel> {
  let selector = scraper::Selector::parse("#tab_content01 a.c-card_link").expect("[BUG] Invalid selector");
  let mut channel = Channel::default();
  channel.set_language("ja".to_string());
  channel.set_title("JAMSTEC BASE".to_string());
  ...
  for elem in doc.select(&selector) {
    ...
    let selector = scraper::Selector::parse("p.c-card_title").unwrap();
